...
public class SparkPartitionUtils {
  ...
  public static Literal<?> toGravitinoLiteral(InternalRow ident, int ordinal, DataType sparkType) {
    ...
    if (sparkType instanceof ByteType) {
      ...
      // Varchar and Char are checked before StringType because Spark 4.0 made both extend it; on
      // 3.3-3.5 they extend AtomicType, so this order is correct for both builds of this file.
  ...
  public static Object getSparkPartitionValue(String hivePartitionValue, DataType dataType) {
    ...
    try {
      if (dataType instanceof ByteType) {
        ...
        // does not matter. Spark 4.0 needs only the first check; 3.3-3.5 need the other two.
